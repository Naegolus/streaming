...
restart_receive:
        ...
        break;
    case recv_data:
        if (byte == '$') //$ must not occur in package data.
...
void stub_init(void) {
    ...
    MINI_UART_CFG = 0x07;
    /* Baud Rate 115200 bit/s at 50 MHz */
    MINI_UART_UBRS_H = 0x1B;
    MINI_UART_UBRS_L = 0x20;

    /* Baud Rate 115200 bit/s at 25 MHz */
    /* MINI_UART_UBRS_H = 0x0D; */
    /* MINI_UART_UBRS_L = 0x90; */

    REGISTER_INTERRUPT(miniuart_isr, DEBUG_INT); //Interrupt-Vectors are stored from -16 to -1.
    REGISTER_TRAP(trap0, 0); // Trap 0. https://trac.ecs.tuwien.ac.at/SCARTS/ticket/3
